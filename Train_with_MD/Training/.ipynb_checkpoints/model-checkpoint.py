...
class MultimodalModel_CrossAttn(nn.Module):
    def __init__(self, num_heads=4, hidden_dim=512, self_meta=False, self_image=False, cross_attn = True):
        super(MultimodalModel_CrossAttn, self).__init__()
        ...
        # Final classification layer
        self.fc_combined = nn.Linear(512 * 2, 5) if self.cross_attn else nn.Linear(512 + 512, 5) # Combine image and metadata features, số lớp là 5, input là 512 * 2 hoặc 512 + 512
...
class MultimodalModel_Fusion(nn.Module):
    def __init__(self, num_heads=4, hidden_dim=512, self_meta=False, self_image=False):
        ...
        # Final classification layer
        self.fc_combined = nn.Linear(512 + 512, 5) # Combine image and metadata features, số lớp là 5, input là 512 + 512
...
def get_model(args):
    if args.model == "cross_attn":
        return MultimodalModel_CrossAttn(self_meta = args.self_meta, self_image = args.self_image, cross_attn = args.cross_attn)
    elif args.model == "fusion":
      return MultimodalModel_Fusion(self_meta = args.self_meta, self_image = args.self_image)